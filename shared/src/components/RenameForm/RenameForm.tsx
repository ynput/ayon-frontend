...
import { checkName, NamingConfig, parseAndFormatName } from '@shared/util'
...
export const RenameForm: React.FC<InlineEditingWidgetProps> = ({
  ...
  const formattedFromLabel = parseAndFormatName(label, config)
  const isNameAutoGenerated = name === formattedFromLabel || !name
  ...
  const handleSave = useCallback(async () => {
    ...
    if (hasChanges) {
      ...
      if (!nameCheck.valid || !name) {
        // Invalid name, do not proceed
        console.error('Invalid name:', nameCheck.error)
        toast.error(`Invalid name: ${nameCheck.error}`)
        return
      }
  ...
  const handleLabelChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (isNameAutoGenerated && !nameDisabled) {
        setName(parseAndFormatName(value, config))
      }
    },
