...
// Single source of truth for the column list — the settings panel and the table's add button
// must offer exactly the same columns.
export const useProjectTableColumnItems = ({
  ...
  const columns: (AddColumnItem & { hidden?: boolean })[] = [
    {
      value: 'thumbnail',
      label: 'Thumbnail',
    },
