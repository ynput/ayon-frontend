...
export const FormFileUpload = (props: FormFileUploadProps) => {
  ...
  const handleFileChange = useCallback(
    (file?: File) => {
      ...
      if (file.size > 2 * 1024 * 1024) {
        // 2 MB limit, because we're sending it in JSON as base64
      ...
      if (file) {
        ...
        reader.onload = () => {
          ...
          props.onChange({ payload: base64String, filename: fileName })
        }
  ...
  return (
    <DropArea onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
      <input
        type="file"
        ref={inputRef}
...
export const FormFileDownload = (props: FormFileDownloadProps) => {
  ...
  return (
    <DownloadLink>
      <LinkButton onClick={handleDownload}>{props.value.filename}</LinkButton>
    </DownloadLink>
  )
}
