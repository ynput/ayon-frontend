...
import { api, ReviewableModel, VersionReviewablesModel } from '../review'
import { getAddonsApi as addonsApi } from '../../addons'
import {
  Summary,
  ...
  endpoints: (build) => ({
    ...
    hasTranscoder: build.query<boolean, undefined>({
      queryFn: async (_arg, { dispatch }) => {
        ...
        const res = await dispatch(addonsApi.endpoints.listAddons.initiate({ details: false }))
