...
export type ActivityFragmentFragment = {
  ...
  files: Array<{ id: string; name: string | null; size: string; mime: string | null }>
  origin: { id: string; name: string; label: string | null; type: string } | null
  reactions: Array<{
  ...
  version: { thumbnailHash: string; status: string; attrib: { comment: string | null } } | null
}
...
export type GetActivityUsersQuery = {
  users: { edges: Array<{ node: { name: string; attrib: { fullName: string | null } } }> }
}
...
export type GetEntitiesChecklistsQuery = {
  project: {
    name: string
    activities: {
      edges: Array<{ cursor: string | null; node: { activityId: string; body: string } }>
    }
  }
}
...
export type GetDetailsPanelVersionQuery = {
  project: {
    ...
    version: {
      ...
      representations: {
        edges: Array<{
          node: { id: string; name: string; fileCount: number; files: Array<{ path: string }> }
        }>
      }
    } | null
  }
...
export const GetActivityUsersDocument = new TypedDocumentString(`
  ...
  users(last: 5000, projects: $projects, isSupport: false) {
    edges {
      node {
        name
        attrib {
...
export const GetKanbanDocument = new TypedDocumentString(`
  ...
  kanban(projects: $projects, assigneesAny: $assignees, last: 5000) {
    edges {
      node {
        ...KanbanFragment
      }
...
const injectedRtkApi = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    GetUpdatedAndNewFolders: build.query<
      GetUpdatedAndNewFoldersQuery,
      GetUpdatedAndNewFoldersQueryVariables
    >({
      query: (variables) => ({
        document: GetUpdatedAndNewFoldersDocument as unknown as string,
        variables,
      }),
    }),
