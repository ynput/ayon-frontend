...
export const getMixedState = (values: any[]): { value: any; isMixed: boolean } => {
  ...
  const allSame = values.every((v) => {
    if (Array.isArray(v) && Array.isArray(values[0])) {
      return v.length === values[0].length && v.every((val, index) => val === values[0][index])
    }
    return v === values[0]
  })
  ...
  if (values[0] && !isNaN(new Date(values[0] as string).getTime())) {
    const latestDate = values.reduce((latest, currentValue) => {
