...
export const loadRemoteCSS = async (
  addonName: string,
  addonVersion: string,
  remoteName: string,
) => {
  ...
  try {
    ...
    if (cssLinkMatch && cssLinkMatch[1]) {
      ...
      link.href = `/addons/${addonName}/${addonVersion}/frontend/modules/${remoteName}/${cssHref.substring(
        2,
      )}`
    ...
    for (const path of fallbackPaths) {
      try {
        ...
        console.warn(
          `Fallback CSS path failed for ${addonName}/${remoteName}:`,
          path,
          fallbackError,
        )
      }
