        ...
        <Fieldset legend="Project statistics">
            <ul>
                {Object.keys(data.counts).map((key) => (
                        <li key={key}>{key} : {JSON.stringify(data.counts[key]) }</li>
                    )
                )}
            </ul>
        ...
        <main>
            <section className="lighter" style={{ flexBasis: "600px", padding: 0}}>
                <div className="wrapper"> 
                    <DataTable 
                        ...
                        onSelectionChange={
                            e => setSelectedProject(e.value)
                        }
                    >
                        <Column field="name" header="Name" />
                        <Column field="createdAt" header="Created" body={rowdata=> DateTime.fromSeconds(rowdata.createdAt).toRelative() } />
                        <Column field="updatedAt" header="Updated" body={rowdata=> DateTime.fromSeconds(rowdata.updatedAt).toRelative() } />
                    </DataTable>
                </div>
            </section>
            ...
            <section style={{ flexGrow: 1}} className="invisible">
                ...
                <section style={{ flexGrow: 1}}>
                    {selectedProject && <ProjectStats projectName={selectedProject.name}/>}
                </section>
